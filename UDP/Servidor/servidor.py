...
def manejarCliente(conn, addr, NOM_ARCHIVO, TAM_ARCHIVO, ID, NUM_CONEXIONES, hashServidor, ip, puerto, servidorUDP):
    ...
    print(servidorUDP)
    data = f'[-] Conexion con el servidor UDP exitosa'
    data = data.encode("utf-8")
    servidorUDP.sendto(data, addr)
...
def main():
    ...
    while (NUM_CLIENTES < NUM_CONEXIONES):
        # SERVIDOR UDP (INICIO)
        hostUDP = socket.gethostbyname(socket.gethostname())
        portUDP = 4455

        serverUDP = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)

        serverUDP.bind((hostUDP, portUDP))
        # SERVIDOR UDP (FIN)
        ...
        thread = threading.Thread(target=manejarCliente, args=(
            conn, addr, NOM_ARCHIVO, TAM_ARCHIVO, NUM_CLIENTES, NUM_CONEXIONES, hashServidor, addr[0], {addr[1]}, serverUDP))
